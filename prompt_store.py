...
def get_data_prompt(prompt: str) -> str:
    return f"""given a sqlite3 database with these tables:
        ...
        When dealing with strings/txt or like conditions, always lower-case the text.
        return sql only"""
...
def get_graph_prompt(result_schema: dict, key_description: str = None) -> str:
    return f"""given the data in a variable named raw_results that adheres to this jsonschema: {result_schema} and is in a list
        give charts.js code that could display the data. pick the most appropriate chart type.
        ...
        use 'var' to declare variables and never 'const' 
        return code only"""

def get_modify_graph_prompt(change_request: str) -> str:
    return f"""{change_request}. give me the complete code block as you did above and do not give notes or advice"""
